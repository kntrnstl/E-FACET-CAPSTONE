...
async function createQrphPayment(req, res) {
  try {
    ...
    // verify schedule exists + matches course
    ...
    await pool.execute(
      `INSERT INTO student_payment_submissions
       ...
       VALUES
        (?, ?, ?, ?,
         ?, 'PHP', 'GCASH', 'FOR_VERIFICATION', NULL, ?, NOW(), NOW())`,
...
// 2) Upload proof (NO reservation yet)
...
const proofUrl = `/uploads/payment_proofs/${req.file.filename}`;
    ...
    const [payRows] = await conn.execute(
      `SELECT id, payment_ref, student_id, status
    ...
    await conn.execute(
      `UPDATE student_payment_submissions
       SET proof_url = ?, status = 'PROOF_SUBMITTED', updated_at = NOW()
       WHERE payment_ref = ? AND student_id = ?`,
